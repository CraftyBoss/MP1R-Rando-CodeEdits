...
//            Logger::log("Command List was Empty! Skipping Render.\n");
        ...
        for (size_t i = 0; i < drawData->CmdListsCount; i++) {
            ...
            for (auto cmd: cmdList->CmdBuffer) {
                ...
                // currently unable to get clipping working properly, more research into why its broken should be done.
...
//
//                Logger::log("Clip Rect: X: %f Y: %f W: %f H: %f\n", clipRect.x, clipRect.y, clipRect.z, clipRect.w);
//                Logger::log("Clip Min: X: %f Y: %f\n", clip_min.x, clip_min.y);
//                Logger::log("Clip Max: X: %f Y: %f\n", clip_max.x, clip_max.y);
//                Logger::log("Clip Size: X: %f Y: %f\n", clip_size.x, clip_size.y);
//
//                if (clip_max.x <= clip_min.x || clip_max.y <= clip_min.y) {
//                    Logger::log("Skipping draw command, clip max/min are invalid.\n");
//                    continue;
//                }
//
//                Logger::log("Setting Scissor to: X: %d Y: %d W: %d H: %d\n", (int)clip_min.x, (int)clip_min.y, (int)clip_size.x, (int)clip_size.y);
